...
public sealed partial class MainWindow : Window
{
    ...
    public MainWindow()
    {
        ...
        if (_uninstall)
        {
            Heading.Text = "解除安裝 Peeklism";
            Description.Text = "移除 Peeklism 及其本機資料。";
            Details.Text = "預設清除設定、使用紀錄與快取。若想在重新安裝後繼續使用原設定，請勾選保留資料。執行中的 Peeklism 會自動關閉。";
        ...
        {
            Heading.Text = "更新 Peeklism";
            Description.Text = "直接更新目前安裝，保留設定、使用紀錄與登入啟動設定。";
            Details.Text = "執行中的 Peeklism 會自動關閉，不需要手動退出。只替換有變動的檔案；更新失敗會嘗試回復原檔案。";
            InstallPath.IsReadOnly = true;
    ...
    private async void ActionClick(object sender, RoutedEventArgs e)
    {
        ...
        try
        {
            ...
            Description.Text = _uninstall ? "Peeklism 已從這台電腦的安裝位置移除。" : "選取檔案後按空白鍵，開始預覽。";
            ...
            Status.Message = _uninstall
                ...
                : "從開始功能表開啟，或按下方按鈕開始使用。";
            Status.IsOpen = true;
            ActionButton.Content = _uninstall ? "完成" : "開啟 Peeklism";
