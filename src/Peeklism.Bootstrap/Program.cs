...
internal static class Program
{
    ...
    private static int Main(string[] args)
    {
        ...
        try
        {
            ...
            root = Path.Combine(Path.GetTempPath(), "Peeklism.Setup", Guid.NewGuid().ToString("N"));
            ...
            var start = new ProcessStartInfo(Path.Combine(work, "Peeklism.Setup.exe"))
            {
        ...
        {
            MessageBox(0, "無法開啟安裝程式。\n" + exception.Message, "Peeklism", IconError);
            return 1;
        }
        finally
        {
    ...
    private static string ChooseLayout(System.IO.Compression.ZipArchive payload, string packages)
    {
        ...
        var answer = MessageBox(
            0,
            "這台電腦還沒有 Peeklism 需要的 Microsoft 元件。要安裝可供其他 App 共用的版本嗎？\n\n"
                + "選「是」可節省安裝空間；選「否」也能繼續安裝 Peeklism。",
            "Peeklism",
            IconQuestion);
        ...
        try
        {
            using var progress = new ProgressDialog("Peeklism", "正在準備必要元件…");
    ...
    {
        using var progress = new ProgressDialog("Peeklism", "正在準備安裝內容…");
