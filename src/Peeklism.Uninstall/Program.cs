...
internal static class Program
{
    ...
    private static int Main(string[] args)
    {
        try
        {
            ...
            var setupSource = Path.Combine(sourceDir, "Peeklism.Setup.exe");
            if (!File.Exists(setupSource))
            {
                MessageBox(0, "找不到 Peeklism.Setup.exe，無法啟動解除安裝程式。", "Peeklism", IconError);
                return 1;
            }
