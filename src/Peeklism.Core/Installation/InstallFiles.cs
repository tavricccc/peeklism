...
{
    public const string ManifestName = "peeklism-install.json";
    ...
    public static InstallManifest ReadManifest(string root)
    {
        ...
        if (manifest.Product != "Peeklism" || manifest.Files is null || manifest.Files.Count == 0)
            throw new InvalidDataException("不是 Peeklism 管理的安裝資料夾。");
