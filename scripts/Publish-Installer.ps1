...
$releaseExe = Join-Path $installerRoot 'Peeklism.Setup.exe'
...
function New-Layout([string]$Destination, [bool]$SelfContainedSdk) {
    ...
    # else. Peeklism calls none of the Windows AI APIs, and these two are loaded only when
    ...
    foreach ($runtimeConfig in Get-ChildItem -LiteralPath $Destination -Filter 'Peeklism.*.runtimeconfig.json' -File) {
    ...
    @{ Product = 'Peeklism'; Version = $Version; Files = $files } | ConvertTo-Json -Depth 5 |
        Set-Content -LiteralPath (Join-Path $Destination 'peeklism-install.json') -Encoding utf8
}
...
try {
    ...
    $assets = Get-Content -LiteralPath (Join-Path $projectRoot 'src/Peeklism.App/obj/project.assets.json') -Raw | ConvertFrom-Json
    ...
    $staged = Join-Path $work 'Peeklism.Setup.exe'
    Copy-Item -LiteralPath (Join-Path $launcher 'Peeklism.Setup.exe') -Destination $staged
    ...
    try {
        ...
        $output.Write([Text.Encoding]::ASCII.GetBytes('PEEKLISM-PAYLOAD'), 0, 16)
    } finally {
        $output.Dispose()
        $source.Dispose()
    }
    ...
    if (Test-Path -LiteralPath $releaseExe) {
        ...
        $archived = Join-Path $history ('Peeklism.Setup-' + (Get-Date -Format 'yyyyMMdd-HHmmss') + '.exe')
